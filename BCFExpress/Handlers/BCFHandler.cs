...
using Point = BCFExpress.Bcf.Bcf2.Point;
...
namespace BCFExpress.Handlers
{
    public class BCFHandler : IExternalEventHandler
    {
        ...
        //        vp.SnapshotImage = GetImageArray(imageFile);
            ...
            try
            {
                switch (1)
                {
                    case ComponentOption.SelectedElements:
                        ...
                        break;
                     //case ImagingUtils.ComponentOption.OnlyVisible:
        ...
        private OrthogonalCamera GetOrthogonalCamera(View3D view, string viewpoint_Guid)
        {
            ...
            try
            {
                ...
                camera.Guid = Guid.NewGuid().ToString();
                camera.ViewPointGuid = viewpoint_Guid;
                var viewPoint = new BCFExpress.Bcf.Bcf2.Point
                {
                    Guid = Guid.NewGuid().ToString(),
                    X = c.X.ToMeters(),
                    Y = c.Y.ToMeters(),
